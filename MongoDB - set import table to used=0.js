   ...
   const fs = require('fs'),
     ...
     */
      ...
      function markimportasNOTUSED(client){
            ...
            log('\n\nWe will mark now the import table as used=0')
            ...
            collection.updateMany({}, { $set: { used: 0 } }, function(e, res) {
                if(e){
                     log(chalk.red.bold('❌ ERROR') + ' #1 There was an error while try to mark the mark the the import table as used=0 - ' + chalk.white.bold('error:\n') + e);
                     return;
                 } // if(e){

                                  log('Successfully set the import table to used=0');
                                  client.close();
                                  process.exit();

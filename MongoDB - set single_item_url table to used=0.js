     ...
     */
          ...
          */
                                ...
                                MongoClient.connect(MongoDB_DB_URL, { useNewUrlParser: true }, function(e, client) {
                                  ...
                                  markimport_single_item_urlasNOTUSED(client);
                                  //client.close();
      ...
      function markimport_single_item_urlasNOTUSED(client){

            log('\n\nWe will mark now the single_item_url table as used=0')



            var collection = MongoDB.collection('single_item_url');
            collection.updateMany({}, { $set: { used: 0 } }, function(e, res) {
                if(e){
                     log(chalk.red.bold('❌ ERROR') + ' #1 There was an error while try to mark the mark the the single_item_url table as used=0 - ' + chalk.white.bold('error:\n') + e);
                     return;
                 } // if(e){

                                  log('Successfully set the single_item_url table to used=0');
                                  client.close();
                                  process.exit();
      ...
      } // function markimport_single_item_urlasNOTUSED(){

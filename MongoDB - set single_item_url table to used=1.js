   ...
   const mysql = require('mysql'),
     ...
     */
      ...
      function markimport_single_item_urlasNOTUSED(client){
            ...
            log('\n\nWe will mark now the single_item_url table as used=1')
            ...
            collection.updateMany({}, { $set: { used: 1 } }, function(e, res) {
                if(e){
                     log(chalk.red.bold('❌ ERROR') + ' #1 There was an error while try to mark the mark the the single_item_url table as used=1 - ' + chalk.white.bold('error:\n') + e);
                     return;
                 } // if(e){

                                  log('Successfully set the single_item_url table to used=1');
                                  client.close();
                                  process.exit();
